...
def get_productions_with_permissions(permissions):
    ...
    return UserService.resources_with_perms(user, permissions, resource_types=['production']).all()
...
@api.model_resource(production, Production, '/fields/details/<int:field_detail_id>/productions', '/fields/details/productions/<int:production_id>')
class ProductionResource(ModelResource):
    ...
    def list(self, **kwargs):
        ...
        print("field authorized: ", productions)
        return self.serializer.dump([get_production_details(production) for production in productions], many=True)
